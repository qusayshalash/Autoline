...
# ---- aggregates --------------------------------------------------------------------
#
# The same numeric field carries a row count and an aggregate over a measure column, so
# how a figure is written has to follow the figure rather than its type.


def test_a_whole_number_never_grows_a_decimal_tail():
    """A count of 40 is "40". It reads as a count, and anything pasting the file into a
    report expects one - the field is a float only because an average also travels here."""
    payload = se.build(request("csv"))
    assert csv_rows(payload)[4][1] == "40"


def test_an_aggregate_keeps_its_decimals():
    req = request(
        "csv",
        rows=[StatisticsExportRow(label=HEBREW, count=2018.37, percentage=40.0)],
        total=None,
    )
    assert csv_rows(se.build(req))[4][1] == "2,018.37"


def test_a_measured_breakdown_has_no_total_row_figure():
    """Averages do not add up. The total row keeps its label and leaves the figure blank
    rather than printing a sum of means."""
    req = request(
        "csv",
        rows=[StatisticsExportRow(label=HEBREW, count=2018.37, percentage=40.0)],
        total=None,
    )
    rows = csv_rows(se.build(req))
    assert rows[-1][0] == ARABIC
    assert rows[-1][1] == ""


def test_the_pdf_renders_a_measured_breakdown():
    """Reportlab is strict about what it will lay out; a float arriving where an int used
    to be must not take the whole document down."""
    req = request(
        "pdf",
        rows=[StatisticsExportRow(label=HEBREW, count=2018.37, percentage=40.0)],
        total=None,
    )
    assert se.build(req).startswith(b"%PDF")


@pytest.mark.parametrize("count,expected", [(40, "#,##0"), (2018.37, "#,##0.00")])
def test_excel_formats_the_column_for_what_it_holds(count, expected):
    """Excel's own display rounds to the format, so a mean written under an integer
    format would show on screen as a year and only reveal its decimals in the formula
    bar."""
    req = request(
        "xlsx",
        rows=[StatisticsExportRow(label=HEBREW, count=count, percentage=40.0)],
        total=None,
    )
    payload = se.build(req)
